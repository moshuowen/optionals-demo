...
//        System.out.println(Arrays.toString(split));
        ...
        String name = "as\s\"([^\"]*)\"";
        String value = "'([^']*)'\sas";
        String count = ",\s\"([^\"]*)\"\sas \"count\"";
        Pattern pattern = Pattern.compile(value);
//        System.out.println(split[0]);
        Matcher matcher1 = pattern.matcher(split[0]);
        List<String> l = new ArrayList<>();
        while (matcher1.find()) {
            l.add(matcher1.group(0).replaceAll(value, "$1"));
        }
        HashMap<Object, Object> map = new HashMap<>();
        map.put("name", l.get(0));
        map.put("queryIndex", l.get(1));
        map.put("queryParam", l.get(2));
        System.out.println(map);
        System.out.println(l);
        for (int i = 0; i < split.length; i++) {
            for (String as : split[i].split("as")) {
                // 获取到count里面的语法内容
                if (as.contains("count")) {
                    if (as.toLowerCase().contains("where")) {
//                        System.out.println(as.substring(as.indexOf("("), as.lastIndexOf(")") + 1)+"\n========"+(i+1));
